...
class PixelObservation(Wrapper):
    def __init__(self, env: dm_env.Environment, render_kwargs: dict | None = None, num_stacked_frames=1):
        super().__init__(env)
    ...
    def step(self, action):
        ...
        self.stacked_frames.append(timestep.observation['pixels'])
        return timestep._replace(observation=np.array(self.stacked_frames))
    
    def reset(self):
        timestep = super().reset()
        self.stacked_frames.extend([timestep.observation['pixels']] * self.num_stacked_frames)
        return timestep._replace(observation=np.array(self.stacked_frames))
